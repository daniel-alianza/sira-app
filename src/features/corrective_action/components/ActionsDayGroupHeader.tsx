...
import type { ActionDayGroup } from '../utils/group-actions-by-weekday';

export interface ActionsDayGroupHeaderProps {
  readonly group: ActionDayGroup;
}

export function ActionsDayGroupHeader({ group }: ActionsDayGroupHeaderProps) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200/90 bg-[#0A2240]/4 px-4 py-3 md:px-6">
      ...
      <span className="rounded-full bg-white px-2.5 py-0.5 text-xs font-medium text-slate-600 shadow-sm">
        ...
        {group.actions.length === 1 ? 'acción' : 'acciones'}
      </span>
    </div>
  );
}
